...
function parseSimulationResult(simulation, label) {
  if (simulation?.error) {
...
async function simulateContractCall({ server, sourceAccountId, sourceAccountSequence, contractId, functionName, args = [] }) {
  ...
  return parseSimulationResult(simulation, `SEP-41 ${functionName}(${contractId})`);
}

async function fetchTokenMetadata({ server, sourceAccountId, sourceAccountSequence, contractId }) {
  const [symbol, name, decimals] = await Promise.all([
    simulateContractCall({ server, sourceAccountId, sourceAccountSequence, contractId, functionName: 'symbol' }),
    simulateContractCall({ server, sourceAccountId, sourceAccountSequence, contractId, functionName: 'name' }),
    simulateContractCall({ server, sourceAccountId, sourceAccountSequence, contractId, functionName: 'decimals' })
  ]);
